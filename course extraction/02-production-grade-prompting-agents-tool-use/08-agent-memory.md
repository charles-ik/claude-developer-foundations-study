...
Sources: [S19](https://anthropic-partners.skilljar.com/content/wp/4hdejjwplbrm/3fpv056zo4wp4/Developer_M2_vF2.html#S19) · [S20](https://anthropic-partners.skilljar.com/content/wp/4hdejjwplbrm/3fpv056zo4wp4/Developer_M2_vF2.html#S20) · [S21](https://anthropic-partners.skilljar.com/content/wp/4hdejjwplbrm/3fpv056zo4wp4/Developer_M2_vF2.html#S21)
...
**Prompt:** A customer support agent assists the same user across daily check-ins over two weeks. Each session starts where the previous one left off.

- **Interactive option:** In-context memory: all state lives in the active conversation.
- **Interactive option:** External storage: write state to a database at session end, then read it back at session start.
- **Interactive option:** No persistent memory (stateless): each session starts fresh.

**Prompt:** A document formatter receives a file, applies a transformation, returns the output, and terminates. Each job is fully independent.

- **Interactive option:** In-context memory: all state lives in the active conversation.
- **Interactive option:** External storage: write state to a database at session end, then read it back at session start.
- **Interactive option:** No persistent memory (stateless): each session starts fresh.

**Prompt:** A coding assistant works with a developer across a multi-hour session. The session will not continue after it ends.
